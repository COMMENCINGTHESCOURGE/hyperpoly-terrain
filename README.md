...
Assets in `assets/` are deployed automatically from [`trench-builder`](https://github.com/COMMENCINGTHESCOURGE/trench-builder). Running `build_all_assets.py` in the trench-builder project copies all generated GLBs here via auto-detection of the sibling `hyperpoly-terrain/assets/` directory. The `game/index.html` loads a subset via GLTFLoader and places them in the world.
